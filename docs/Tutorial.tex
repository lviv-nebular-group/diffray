...
{\it opacity on/off} - turn on/off opacity calculation (turning this off assumes that environment is very opticlly thin for all
quantum) \\
{\it continuum on/off} - turn on/off radiative transfer for continuum. Turning off will make DiffRay to calculate only line fluxes \\
{\it abundance on/off} - turn on/off calculation of weighted elemental abundances within volume cut by aperture/along observer sightline. Once you turn this on,
abundance data files are required (See \ref{dataAbund} for details). \\
{\it overview on/off} - turn on/off calculation of average electron temperature and density within volume cut by aperture/along observer sightline. Once you turn this on,
